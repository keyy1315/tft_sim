...
> ⚠️ **17.5 patch pending** (데이터 미반영, [[patch-17-5]]): `AreaDamage` ★3 190 → **215** (buff) / AttackSpeed 0.85 → **0.8** (nerf).
...
- **불한당** (`TFT17_AssassinTrait`, bp 2/3/4/5): synergy scaling 경로(`:579` "불한당: AD/AP 획득")로 AD/AP 가산 (scaling.json 기반).

## sim 통합 상태 — `partial`

✅ **활성**:
- stats 17.4 정합 (hp 750, armor/MR 45, AD 50, AS 0.85, range 2, mana 0/30)
...
- calibration: game-423/424 **부재(미측정)**. 17.5 AreaDamage buff/AS nerf 데이터 미반영([[patch-17-5]]).
