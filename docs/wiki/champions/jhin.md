...
sources:
  ...
  - "src/lib/simulator/systems/ability.ts:253 (TFT17_Jhin: { pattern: 'multi', maxTargets: 3, debuff: { armorReduction: 15, duration: 4 }, hitCount: 4, damageVar: 'APDamage' })"
...
```ts
TFT17_Jhin: { pattern: 'multi', maxTargets: 3, debuff: { armorReduction: 15, duration: 4 }, hitCount: 4, damageVar: 'APDamage' }
```
...
> ⚠️ active 의 핵심 직격(`ADDamage` scaleAD)이 `damageVar: 'APDamage'` 로 ~10배 under. 5코 carry 의 active DPS 가 크게 과소. **var 오선택(APDamage→ADDamage) 확정 — sim fix 우선 후보 (별도 PR: ability.ts:253 + 회귀 가드 + diff-cache)**.
...
|-----------|------------|------|
| **main pipeline** | ✅ active multi 4-hit (APDamage) + armorReduction debuff | `ability.ts:253`, `combatLoop.ts:6594` (findAbilityTargets multi) |
...
- active 잔상 손 multi 4-hit (단 damageVar APDamage 오선택)
...
|---|------|------|------|---------------------|------|
| P0 | active damageVar APDamage 오선택 (확정) | config `damageVar: 'APDamage'` [0,4,6,44] vs raw 주력 `ADDamage` [0,41,62,644] (desc `TotalDamage` scaleAD). ~10배 under | **P0** | cast config — `damageVar: 'APDamage'` → `'ADDamage'` (scaleAD 주력). APDamage 는 부차(scaleAP) | 5코 carry active DPS ~10배 과소. sim fix 우선 후보 |
...
> 📌 **passive(고정 AS + AS→AD 전환) + 3 trait 는 sim 반영**. `partial` 사유는 **active damageVar APDamage 오선택 P0** (확정 sim 회귀, 주력 ~10배 under, sim-fix 별도 PR) + armorReduction/FinalShot/per-target 감소 P2. passive 시그니처는 정합.
...
- [x] **본문 Lint P0 1건(damageVar APDamage — 확정 sim 회귀, sim-fix 별도 PR) + P2 4건 등록 → `sim_active: partial` 유지** (룰 #15: P0 lint case 등록 + partial 강등 정합)
- [ ] **sim fix (별도 PR)**: active damageVar APDamage→ADDamage (P0 확정) / armorReduction / FinalShot / per-target 감소 (P2)
...
- under-damage calibration (메모리 `project_underdamage_calibration`) — Jhin active damageVar APDamage 오선택 = 5코 carry under-damage 사례
