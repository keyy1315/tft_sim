...
status: LIVE
last_verified: 2026-06-01 (sequence C-5a — Stargazer Fountain Periodic Heal 매핑 풀림 + (별돌보미 추가) 2.5→3 너프 + sim 통합. **sequence C-5 완료** (5/5 — Mountain PR #169 / Serpent PR #170 / Huntress PR #171 / Fountain ADAP PR #172 / Fountain Heal 본 PR). 잔존: sequence B 후속 (DarkStar / NOVA Aatrox 등) + sequence D (Arbiter + Psionic))
sources:
...
| **sim 코드** (`combatLoop.ts` trait helpers + ability config) | ⚠️ **자동 반영 + hardcoded 잔존**. raw json 갱신만으로 자동 sim 반영: Shen passive damage / Zed-Shen mana / Sniper amp / Challenger AS ✅ (4건). **sequence C 적용 완료**: ✅ JaxCarry damage (PR #164) / ✅ Galio selfBuff.durability star별 read + OOR durability 분기 fix (PR #165) / ✅ NOVA Akali Bleed + Kindred damageAmp + Mark Timer (PR #166) / ✅ SpaceGroove EffectBonus helper 통합 (PR #167) / ✅ Stargazer Mountain AS/DR 너프 (PR #169 sequence C-5d) / ✅ Stargazer Serpent ★1 Poison 너프 (PR #170 sequence C-5b) / ✅ Stargazer Huntress AS/Teamwide/NumMarks 너프+강화 (PR #171 sequence C-5c) / ✅ Stargazer Fountain StackingADAP 매핑 풀림 + (5) 7→9 너프 + sim 통합 (PR #172 sequence C-5e) / ✅ **Stargazer Fountain Periodic Heal 매핑 풀림 + (별돌보미 추가) 2.5→3 너프 + sim 통합 (C-5a, 본 PR)** — raw `{d7e6d620}` = teamwide periodic heal (1% per 2s, 모든 tier 동일) / raw `{f2840aed}` = 별돌보미 추가 periodic heal (17.4 너프 2.5→3% per 2s). `applyStargazerEffects` Fountain 분기에 두 read 추가 + `fountainHealPctPerTick` 합산 set (teamwide + 별돌보미 ownerExtra). main loop fountain tick (line 5352-5356) 의 기존 heal 적용 코드 활용. 별도 hardcoded 잔존: Jax selfBuff.durability (raw FlatDR 단위 변환 필요) / NOVA Aatrox shredPct (raw 매핑 verify 필요) | **sequence C** (9/9 → 약 100% 진행, **C-5 sub 5건 모두 완료** 🎉) — Stargazer 4 변종 sim 100% 정합 |
...
| **스타게이저** | **Mountain 변종** AS `10%` → **`8%`** + DR `6%` → **`5%`** ✅, **Serpent 변종** Damage (3) tier `25%` → **`20%`** ✅, **Huntress 변종** Teamwide AS `15%` → **`12%`** + Stargazer AS `12/35/55` → **`15/45/70`** + Mark (7) tier `7` → **`9`** ✅, **Fountain 변종** (5) tier StackingADAP `7%` → **`9%`** ✅ + Fountain periodic heal (별돌보미 추가) `2.5%` → **`3%`** ✅ (raw `{13a2a786}`/`{f2840aed}`/`{d7e6d620}` 매핑 풀림 + sim 통합) | [[stargazer-fountain]] + `applyStargazerEffects` Mountain/Serpent/Huntress/Fountain 분기 (`combatLoop.ts:3220-3420`) + main loop fountain tick (`:5333-5363`) ✅ — **sequence C-5 완전 통합** |
