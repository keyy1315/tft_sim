...
| **sim 코드** (`combatLoop.ts` trait helpers + ability config) | ⚠️ **자동 반영 + hardcoded 잔존**. raw json 갱신만으로 자동 sim 반영: Shen passive damage / Zed-Shen mana / Sniper amp / Challenger AS ✅ (4건, PR #163 Codex P2 정정 후 — SpaceGroove EffectBonus 자동 반영 ❌ 정정: helper 미구현). **sequence C 적용 완료**: ✅ JaxCarry damage (PR #164) / ✅ Galio selfBuff.durability star별 read + OOR durability 분기 fix (PR #165) / ✅ NOVA Akali Bleed + Kindred damageAmp + Mark Timer (PR #166). 별도 hardcoded 갱신 필요: Jax selfBuff.durability (raw FlatDR 단위 변환 필요, 도메인 verify 후 별도 PR) / Stargazer 필드 매핑 / SpaceGroove EffectBonus helper 통합 / NOVA Aatrox shredPct (raw 매핑 verify 필요) | **sequence C** (3/5 + 잔존 → 약 60% 진행) — helper 코드 hardcoded 갱신 + 테스트 회귀 |
