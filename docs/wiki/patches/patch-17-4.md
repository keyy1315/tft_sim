...
status: LIVE
last_verified: 2026-06-01 (sequence C-5e — Stargazer Fountain StackingADAP 매핑 풀림 + (5) 7→9 너프 + sim 통합. C-5d Mountain (PR #169) + C-5b Serpent (PR #170) + C-5c Huntress (PR #171) + 본 PR 누적 4/5. sequence C-5a Fountain Healing periodic 및 sequence B 후속 + D 대기)
sources:
...
| **스타게이저** | **Mountain 변종** AS `10%` → **`8%`** + DR `6%` → **`5%`** ✅, **Serpent 변종** Damage (3) tier `25%` → **`20%`** ✅, **Huntress 변종** Teamwide AS `15%` → **`12%`** + Stargazer AS `12/35/55` → **`15/45/70`** + Mark (7) tier `7` → **`9`** ✅, **Fountain 변종** (5) tier StackingADAP `7%` → **`9%`** ✅ (raw `{13a2a786}` 매핑 풀림 + sim 통합), Fountain Healing periodic `2.5%` → **`3%`** (C-5a 대기) | [[stargazer-fountain]] + `applyStargazerEffects` Mountain/Serpent/Huntress/Fountain 분기 (`combatLoop.ts:3220-3406`) + main loop fountain tick (`:5333-5363`) ✅ — Fountain Healing periodic 영역만 별도 |
