...
status: LIVE
last_verified: 2026-06-01 (sequence C-5c — Stargazer Huntress AS/Teamwide/NumMarks 17.4 raw json 적용 완료. C-5d Mountain (PR #169) + C-5b Serpent (PR #170) + 본 PR 누적 3/5. sequence C-5a/e 및 sequence B 후속 + D 대기)
sources:
...
| **sim 코드** (`combatLoop.ts` trait helpers + ability config) | ⚠️ **자동 반영 + hardcoded 잔존**. raw json 갱신만으로 자동 sim 반영: Shen passive damage / Zed-Shen mana / Sniper amp / Challenger AS ✅ (4건). **sequence C 적용 완료**: ✅ JaxCarry damage (PR #164) / ✅ Galio selfBuff.durability star별 read + OOR durability 분기 fix (PR #165) / ✅ NOVA Akali Bleed + Kindred damageAmp + Mark Timer (PR #166) / ✅ SpaceGroove EffectBonus helper 통합 (PR #167) / ✅ Stargazer Mountain AS/DR 너프 (PR #169 sequence C-5d) / ✅ Stargazer Serpent ★1 Poison 너프 (PR #170 sequence C-5b) / ✅ **Stargazer Huntress AS/Teamwide/NumMarks 17.4 (C-5c, 본 PR)** — raw json 만 변경 (sim helper `applyStargazerEffects` Huntress 분기가 raw `Huntress_AS_Teamwide`/`Huntress_AS`/`NumMarks` 직접 read 라 자동 반영). 별도 hardcoded 갱신 필요: Jax selfBuff.durability (raw FlatDR 단위 변환 필요) / Stargazer Fountain 필드 매핑 / NOVA Aatrox shredPct (raw 매핑 verify 필요) | **sequence C** (7/8 + 잔존 → 약 88% 진행, C-5 sub 5건 중 3건 완료) — helper 코드 hardcoded 갱신 + 테스트 회귀 |
...
| **스타게이저** | **Mountain 변종** AS `10%` → **`8%`** + DR `6%` → **`5%`** ✅, **Serpent 변종** Damage (3) tier `25%` → **`20%`** ✅, **Huntress 변종** Teamwide AS `15%` → **`12%`** + Stargazer AS `12/35/55` → **`15/45/70`** + Mark (7) tier `7` → **`9`** ✅, Fountain Healing `2.5%` → **`3%`** (C-5a 대기), Fountain (5) ADAP `7%` → **`9%`** (C-5e 대기) | [[stargazer-fountain]] + `applyStargazerEffects` Mountain/Serpent/Huntress 분기 (`combatLoop.ts:3220-3340`) ✅ — Fountain 영역 별도 |
