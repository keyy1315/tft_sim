...
| [[patch-17-4]] (2026-05-27) | **2건 변경** (PR #162 codex P1 catch — 이전 "carry 변경 없음" 잘못): (1) **JaxCarry damage `170/250/450` → `160/240/420`** 너프 (Reach for the Stars augment 표 명시). (2) base Jax `FlatDR` AP scaling 제거 (`15/20/25 AP` → `20/25/30` flat) — 간접 영향 | ✅ **(1) JaxCarry damage sim 적용 완료** (PR #164 sequence C): `carryAugments.ts:209` damage `[170, 250, 450]` → `[160, 240, 420]` + `hero-augment-stat-system.test.ts:70-75` test assertion 갱신. **★ 실 sim damage 감소 적용** — JaxCarry 전용 cast damage 분기 (`combatLoop.ts:6879-6899` main + `:7212-7240` OOR, PR #140 Lint #11-A + PR #147) 가 `carryCfg.abilityData.damage` 를 cast target 에 적용. self_buff pattern 의 self-hit 가드 (rawAbilityDmgBase=0) 와 **별개 분기** — PR #164 Codex P2 catch 후 정정. (2) base Jax FlatDR 은 sim dead 상태 — sequence C 2번째 PR 에서 `selfBuff.durability` hardcoded 0.3 → star별 raw FlatDR read 로 전환 예정 ([[jax]] G1) |
