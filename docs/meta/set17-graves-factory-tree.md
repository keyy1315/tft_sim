...
라운드 2 children = 4 노드 (흡수 임플란트 / 산탄 사격 / 용융 관통 / 긴급 보호막)
...
> **초크** 는 맹공 path 안에서도 두 갈래로 도달 가능 (흡수 임플란트+ / 산탄 사격) +
> 위력 path (철갑탄) 까지 총 **3 parent**.

### 🟥 위력 프레임 path (#622B22, 19 노드)

라운드 2 children 후보 = 5 노드 (폭발 반경 / 심장추적자 / 탱커 파괴자 / 냉각수 / 분열).
※ 5개 중 4개 random pick 가능성 — 인게임 검증 필요.
...
라운드 2 children = 4 노드 (정밀 조준경 / 한 발에 두 놈 / 파쇄 탄환 / 파편탄)
...
> ⚠️ **트리 정확도 주의**: 위 구조는 사용자 인게임 검수 + lolchess HTML 기반.
> 위력 path 의 round-2 candidates 5개 (4개 random pick 가능성) 등 일부 분기는
> playtest 추가 검증 필요. 신규 weapon 등장 / 기존 weapon 위치 변경 시 본 doc 갱신.
...
| 4 |  | 초크 | Choke | 5 | 투사체 분산 75% 감소 (※ multi-parent: 흡수 임플란트+ / 산탄 사격 / 철갑탄) |
