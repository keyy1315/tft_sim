...
유닛 — 프렐요드 포탑(최대 2개), 자이르 모래 병사(최대 2개), 티버, 보이저, 셴 분신.
`resolveTraits` 는 사용자 배치 유닛 + 자동 유닛 모두 받아서 카운트하므로 자동 유닛이
같은 apiName 을 여러 개 가질 때 dedupe 영향 가능.

`src/data/specialUnits.ts` 확인 결과:

| 자동 유닛 | apiName | traits | 동시 최대 | dedupe 영향 |
|---|---|---|---|---|
| 얼어붙은 포탑 | `TFT16_FreljordTurret` | `[]` | 2 | 없음 (trait 없음) |
| 모래 병사 | `TFT16_AzirSoldier` | `[]` | 2 | 없음 |
| 티버 | `TFT16_AnnieTibbers` | `['비전 마법사']` | 1 | 없음 (단일) |
| 비아와 바이엔 | `TFT17_Summon` | `[]` | 가변 | 없음 |
| 유물 (셴 분신) | `TFT17_ShenProp` | `[]` | 가변 | 없음 |

trait 가진 자동 유닛은 티버 1종뿐이고 단일 등장이라 dedupe 키 충돌 무관. 그 외 자동
유닛은 모두 `traits: []` 라 영향 없음. 결론: 자동 유닛은 dedupe 안전.
